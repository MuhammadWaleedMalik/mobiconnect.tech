...
const Terms = () => {
  ...
  const pageContent =
    languageMap[(currentLanguage?.code as LanguageCode) || 'en'] || languageMap.en;
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4 sm:px-6 md:px-8">
          ...
          >
            {processText(pageContent?.hero?.title || 'Terms and Conditions')}
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 bg-gray-800">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">{processText(pageContent?.team?.title || 'Our Legal Team')}</h2>
      ...
      {/* Key Terms Section with Alternating Image-Text */}
            ...
            >
              <div className="w-full h-full md:w-1/2">
                <img
                  ...
                  alt={item.title || 'Key Term'}
                  className="w-full h-full sm:h-56 md:h-64 object-contain rounded-lg shadow-lg"
                />
              </div>
              <div className="w-full md:w-1/2 p-4 sm:p-6">
                <h3 className="text-xl sm:text-2xl font-bold mb-4">{processText(item.title || 'Untitled Term')}</h3>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-12">
            {(pageContent?.additional?.items || []).map((item, index) => (
              ...
              >
                <h3 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-4">{processText(item.title || 'Untitled Condition')}</h3>
      ...
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 bg-gradient-to-br from-gray-900 to-gray-800 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 sm:mb-6">{processText(pageContent?.cta?.title || 'Contact Our Legal Team')}</h2>
